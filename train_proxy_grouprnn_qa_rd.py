...
import rnn.QualityRd as rnnQuality
...
#             continue
        ...
        if MULTI_PROC:
            ...
            slvQs[slvId].put([(GroupP2PDeterQaRNN, trcs, vi, p2p, None, result_dir), {"modelPath" : modelPath, "expId" :started}])
        else:
#             rnnAgent.setSlaveId(slvId)
#             rnnQuality.setSlaveId(slvId)
            trcs = traces[traceStart:] + traces[:traceStart]
            runExperiments(GroupP2PDeterQaRNN, trcs, vi, p2p, None, result_dir, modelPath=modelPath)
            finished += 1
