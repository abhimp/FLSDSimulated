...
class GroupP2PDeterAgRNN(GroupP2PDeter):
...
def experimentGroupP2PBig(traces, vi, network):
    ...
    for x, nodeId in enumerate(network.nodes()):
        ...
        trace = traces[idx]
        env = GroupP2PDeterAgRNN(vi= vi, traces= trace, simulator= simulator, grp=grp, peerId=nodeId, abr=BOLA, modelPath="ResModelPathRNNQa")
...
def experimentGroupP2PSmall(traces, vi, network):
    ...
    for trx, nodeId, startedAt in [( 5, 267, 107), (36, 701, 111), (35, 1800, 124), (5, 2033, 127)]:
        trace = traces[trx]
        env = GroupP2PDeterAgRNN(vi, trace, simulator, None, grp, nodeId)
