...
def measureQoE(bitrates, qualityLevels, stallTimes, startUpDelay, reward=True):
    ...
    avgQualityVariation = abs(qualityPlayed[-1] - qualityPlayed[-2])
    if not reward:
        avgQl = sum(qualityPlayed)*1.0/len(qualityPlayed)
        avgQualityVariation = sum([abs(bt - qualityPlayed[x - 1]) for x,bt in enumerate(qualityPlayed) if x > 0])/(len(qualityPlayed) - 1)
